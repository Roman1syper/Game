...
from text import fire_mag_t2_1, fire_mag_t2_2, moon_t
from main import *
from anim import NPS_CRYSTAL, NPS_MOON, NPS_FIRE_MAG, NPS_FIRE_MAG2, NPS_FIRE_MAG3, NPS_FIRE_MAG4, fire_mag_icon,fire_mag_icon2,fire_mag_icon3
import json
pygame.init()
...
def main5(hero,screen,death_list):
    global level
    #print(5)
    level = []
    ...
    for row in level: # вся строка
        for col in row: # каждый символ
            if col == "-":
                ...
                entities4.add(pf)
                platforms.append(pf)
            elif col == "M" and death_list['fire_mag_win'] != 1 and death_list['fire_mag_quest'] == 1 and death_list['fire_mag_win2'] != 1 and death_list['location'] < 15:
                if (death_list['worm1_win'] == 1 or death_list['people1-2_win'] == 1) and (death_list['worm2_win'] == 1 or death_list['people2-2_win'] == 1) and (death_list['worm3_win'] == 1 or death_list['people3-2_win'] == 1):
                    fire_mag = Dialogue_img(x - 16,y - 32,135,105,NPS_FIRE_MAG2,NPS_FIRE_MAG4,0,fire_mag_icon3,fire_mag_icon2,0,66,27,3,'fire_mag',fire_mag_t2_2,True,False,True,145,105)
                else:
                    fire_mag = Dialogue_img(x - 16,y - 32,135,105,NPS_FIRE_MAG,NPS_FIRE_MAG3,0,fire_mag_icon,fire_mag_icon2,0,6,27,3,'fire_mag',fire_mag_t2_1,True,False,True,145,105)
    ...
    for character in fait_character:
        if character.character != 'fire_mag':
            ...
            character.win = death_list[f'{character.character}_win2']
                    ...
                    for character in fait_character:
                        if pygame.sprite.collide_rect(g,character):
                            if character.death != 1 and character.win != 1:
                                ...
                                if character.character == 'fire_mag':
                                    death_list[f'{character.character}_win2'] = test[1]
                                else:
