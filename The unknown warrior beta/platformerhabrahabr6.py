...
from text import witch_t3, fire_mag_t3, assistant_t3, knight_t
from main import *
from anim import NPS_CRYSTAL, NPS_ASSISTANT,NPS_ASSISTANT2,NPS_ASSISTANT3, NPS_FIRE_MAG,NPS_FIRE_MAG3, NPS_WITCH,NPS_WITCH2, NPS_KNIGHT2,NPS_KNIGHT2_2, witch_icon,witch_icon2, fire_mag_icon,fire_mag_icon2, assistant_icon,assistant_icon2,assistant_icon3, knight2_icon,knight2_icon2
pygame.init()
...
def main6(hero,screen,death_list):
    global level
    #print(6)
    level = []
    ...
    for row in level: # вся строка
        for col in row: # каждый символ
            if col == "-":
                ...
                entities.add(pf)
            elif col == "_" and death_list['knight_win'] != 1:
                pf_k = Img_stat(x,y,320,32,'assets/other/not_in.png')
                platforms.append(pf_k)
            elif col == " ":
                random_grass += 1
                number_grass = 0
                ...
                if random_stone % death_list['random'][4] == 0:
                    ...
                    entities.add(pf)
                    platforms.append(pf)
            elif col == "W" and death_list['witch_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['witch_dialogue'] == 2:
                w = Dialogue_img(x - 16,y - 32,64,96,NPS_WITCH,NPS_WITCH2,0,witch_icon,witch_icon2,0,5,27,4,'witch',witch_t3)
                animatedEntities2.add(w)
                entities2.add(w)
                fait_character.append(w)
            elif col == "F" and death_list['fire_mag_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['fire_mag_quest'] == -1 and death_list['witch_dialogue'] == 2:
                fire_mag = Dialogue_img(x - 16,y - 32,135,105,NPS_FIRE_MAG,NPS_FIRE_MAG3,0,fire_mag_icon,fire_mag_icon2,0,6,27,3,'fire_mag',fire_mag_t3,True,False,True,145,105)
                animatedEntities2.add(fire_mag)
                entities2.add(fire_mag)
                fait_character.append(fire_mag)
            elif col == "A" and death_list['assistant_death'] != 1 and death_list['ninja_death'] == 1 and death_list['assistant_quest3'] != 1:
                assistant = Dialogue_img(x - 16,y - 16,96,96,NPS_ASSISTANT, NPS_ASSISTANT2, NPS_ASSISTANT3, assistant_icon,assistant_icon2,assistant_icon3 ,10,28,3,'assistant',assistant_t3,False,False,True,96,96)
                animatedEntities2.add(assistant)
                entities2.add(assistant)
                fait_character.append(assistant)
            elif col == "K" and death_list['knight_win'] != 1:
                knight = Dialogue_img(x - 32,y - 16,200,105,NPS_KNIGHT2,NPS_KNIGHT2_2,0, knight2_icon,knight2_icon2,0, 17,28,3,'knight',knight_t,True,False,True,200,105)
                animatedEntities2.add(knight)
                entities2.add(knight)
                fait_character.append(knight)
            #elif col == "K":
            #    knight = Anim_img2(x - 20,y + 4,160,84,NPS_KNIGHT,NPS_KNIGHT2,4)
            #    entities2.add(knight)
            #    list_knight.append(knight)
            #    animatedEntities2.add(knight)
    ...
    for character in fait_character:
        ...
        if character.character == 'witch' or character.character == 'fire_mag' or character.character == 'assistant':
            character.quest = death_list[f'{character.character}_quest3']
        else:
            character.quest = death_list[f'{character.character}_quest']
    if death_list['witch_win'] == 1 and death_list['fire_mag_win'] == 1 and death_list['assistant_win'] == 1:
    ...
    while not hero.winner:
                    ...
                    for character in fait_character:
                        if pygame.sprite.collide_rect(g,character):
                            if character.death != 1 and character.win != 1:
                                ...
                                if death_list['knight_win'] == 1:
                                    ...
                                    if character.character == 'witch':
                                        death_list['witch_quest3'] = 1
                                        w.quest = 1
                                    if character.character == 'fire_mag':
                                        death_list['fire_mag_quest3'] = 1
                                        fire_mag.quest = 1
                            ...
                            if character.character == 'assistant':
                                if death_list['witch_deatch'] == 1 and death_list['fire_mag_deatch'] == 1 and death_list['knight_deatch'] == 1:
        ...
        #for i in list_knight:
        #    if pygame.sprite.collide_rect(hero,i) and i.win != 1 and i.type != 5:
        #        test = fait(screen,hero,7)
        #        i.win = test[1]
        #        hero.health = test[0] - 1
        #        if hero.health <= 0:
        #            hero.end = 3
        #        hero.kill_knight += test[1]
        #        right = False
        #        left = False
        #        down = False
        #        up = False
        ...
        #        pygame.mixer.music.set_volume(hero.musec)
        #        pygame.mixer.music.play(-1, 0.0, 5000)
        if death_list['witch_dialogue'] != 3 and death_list['knight_win'] == 1: #and death_list['assistant_quest'] == 1
            death_list['witch_dialogue'] = 3
        camera.update(hero) 
