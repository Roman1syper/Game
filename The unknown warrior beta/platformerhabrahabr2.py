...
from text import assistant_2t, ninja_t, bro_t, bro2_t, moon_t
from main import *
from anim import NPS_MOON,NPS_ASSISTANT,NPS_ASSISTANT2,NPS_ASSISTANT3, NPS_NINJA,NPS_NINJA2, NPS_CRYSTAL, NPS_BRO1,NPS_BRO2, icon_ninja,icon_ninja2, assistant_icon,assistant_icon2,assistant_icon3, icon_eye, icon_eye2
import json
pygame.init()
...
def main2(hero,screen,death_list):
    global level
    #print(2)
    level = []
    ...
    for row in level: # вся строка
        for col in row: # каждый символ
            if col == "-":
                ...
                pf_f = Img_stat(x,y - 1030,32,1000,'assets/other/not_in.png')
                platforms.append(pf_f)
                fait_pf.append(pf_f)
            elif col == "*" and death_list['assistant_death'] != 1 and death_list['ninja_death'] != 1 and death_list['ninja_evil_win'] != 1:
                ...
                if random_mushroom3 % death_list['random'][3][0] == 0:
                    if random_mushroom3 >= 30 and random_mushroom3 <= 40:
                ...
                moon = Dialogue_img(x,y,100,140,NPS_MOON,NPS_MOON,0,'assets/other/not_in.png','assets/other/not_in.png',0,0,30,3,'moon',moon_t)
                animatedEntities2.add(moon)
                entities2.add(moon)
            elif col == "A" and death_list['assistant_death'] != 1 and death_list['ninja_death'] != 1 and death_list['ninja_evil_win'] != 1:
                assistant = Dialogue_img(x - 16,y - 16,96,96,NPS_ASSISTANT, NPS_ASSISTANT2, NPS_ASSISTANT3, assistant_icon,assistant_icon2,assistant_icon3 ,10,28,1,'assistant',assistant_2t,False,False,True,96,96)
                animatedEntities2.add(assistant)
                entities2.add(assistant)
                fait_character.append(assistant)
            elif col == "N" and death_list['assistant_death'] != 1 and death_list['ninja_death'] != 1 and death_list['ninja_evil_win'] != 1:
                ninja = Dialogue_img(x - 21,y - 16,84,72,NPS_NINJA,NPS_NINJA2,0,icon_ninja,icon_ninja2,0,2,28,1,'ninja',ninja_t,False,False,True,84,72)
                animatedEntities2.add(ninja)
                entities2.add(ninja)
                fait_character.append(ninja)
            elif col == "o":
                bro1 = Dialogue_img(x - 16,y - 15,60,57,NPS_BRO1, NPS_BRO2, 0, icon_eye,icon_eye2,0,11,28,2,'eye1',bro_t)
                animatedEntities2.add(bro1)
                entities2.add(bro1)
                fait_character.append(bro1)
            elif col == "O":
                bro2 = Dialogue_img(x - 16,y - 15,60,57,NPS_BRO1, NPS_BRO2, 0, icon_eye,icon_eye2,0,11,28,2,'eye2',bro2_t,True)
                animatedEntities2.add(bro2)
                entities2.add(bro2)
                fait_character.append(bro2)
    ...
    for character in fait_character:
        ...
        if 'eye' in character.character:
            character.quast2 = death_list[f'{character.character}_quest2']
        ...
        for e in pygame.event.get():
            ...
            if e.type == KEYDOWN and e.key == K_d and hero.pause == -1:
                if hero.attacking == False:
                    ...
                    if pygame.sprite.collide_rect(g,moon):
                        moon.win = 1
                        moon.dialogue = 1
                        death_list['moon'] = 2
                    ...
                    for character in fait_character:
                        if pygame.sprite.collide_rect(g,character):
                            if character.death != 1 and character.win != 1:
                                ...
                                if character.character == 'eye1':
                                    if death_list['eye2_death'] != 1:
                                        bro2.quest = 1
                                        bro2.rect.x = bro1.rect.x - 120
                                        bro2.rect.y = bro1.rect.y 
                                    for i in fait_pf:
                                        i.rect.y += 1000
                                if character.character == 'eye2':
                                    if death_list['eye1_death'] != 1:
                                        bro1.quest = 1
                                        bro1.rect.x = bro2.rect.x + 120
                                        bro1.rect.y = bro2.rect.y 
                                    for i in fait_pf:
                                        i.rect.y += 1000
                                right = False
                                left = False
