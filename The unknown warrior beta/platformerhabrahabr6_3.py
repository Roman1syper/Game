...
from text import witch_t5, fire_mag_t5, assistant_t5, knight2_t, knight3_t, mag_t
from main import *
from anim import NPS_CRYSTAL, NPS_MAG,NPS_MAG2, NPS_ASSISTANT,NPS_ASSISTANT2,NPS_ASSISTANT3, NPS_FIRE_MAG,NPS_FIRE_MAG3, NPS_WITCH,NPS_WITCH2, NPS_KNIGHT,NPS_KNIGHT_2, witch_icon,witch_icon2, fire_mag_icon,fire_mag_icon2, assistant_icon,assistant_icon2,assistant_icon3, knight_icon,knight_icon2, mag_icon,mag_icon2
pygame.init()
...
def main6_3(hero,screen,death_list):
    global level
    level = []
    ...
    fait_knight = 7
    fait_mag = 8
    if death_list['assistant_quest3'] == 1:
        fait_mag = 810
        death_list['knight2_win'] = 1
        death_list['knight3_win'] = 1
        death_list['knight4_win'] = 1
        death_list['knight5_win'] = 1
    if death_list['fire_mag_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['fire_mag_quest'] == -1 and death_list['assistant_quest3'] != 1:
        fait_knight = 77
    if death_list['witch_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['assistant_quest3'] != 1:
        fait_mag = 85
    for row in level: # вся строка
        for col in row: # каждый символ
            if col == "-":
                ...
                pf_knight = Img_stat(x,y,1280,32,'assets/other/not_in.png')
                platforms.append(pf_knight)
            elif col == "*":
                ...
                if random_stone % death_list['random'][4] == 0:
                    ...
                    entities.add(pf)
                    platforms.append(pf)
            elif col == "W" and death_list['witch_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['assistant_quest3'] != 1:
                w = Dialogue_img(x - 16,y - 32,64,96,NPS_WITCH,NPS_WITCH2,0,witch_icon,witch_icon2,0,5,27,4,'witch',witch_t5)
                animatedEntities2.add(w)
                entities2.add(w)
                not_fait_character.append(w)
            elif col == "F" and death_list['fire_mag_win'] != 1 and death_list['assistant_quest'] == 1 and death_list['fire_mag_quest'] == -1 and death_list['assistant_quest3'] != 1:
                fire_mag = Dialogue_img(x - 16,y - 32,135,105,NPS_FIRE_MAG,NPS_FIRE_MAG3,0,fire_mag_icon,fire_mag_icon2,0,6,27,3,'fire_mag',fire_mag_t5,False,False,True,145,105)
                animatedEntities2.add(fire_mag)
                entities2.add(fire_mag)
                not_fait_character.append(fire_mag)
            elif col == "A" and death_list['assistant_death'] != 1 and death_list['assistant_quest3'] != 1 and death_list['ninja_win'] == 1:
                assistant = Dialogue_img(x - 16,y - 16,96,96,NPS_ASSISTANT, NPS_ASSISTANT2, NPS_ASSISTANT3, assistant_icon,assistant_icon2,assistant_icon3 ,10,28,3,'assistant',assistant_t5,False,False,True,96,96)
                animatedEntities2.add(assistant)
                entities2.add(assistant)
                not_fait_character.append(assistant)
            elif col == "a" and death_list['assistant_death'] != 1 and death_list['assistant_quest3'] == 1:
                assistant = Dialogue_img(x - 16,y - 16,96,96,NPS_ASSISTANT, NPS_ASSISTANT2, NPS_ASSISTANT3, assistant_icon,assistant_icon2,assistant_icon3 ,10,28,3,'assistant',assistant_t5,True,False,True,96,96)
                animatedEntities2.add(assistant)
                entities2.add(assistant)
                fait_character.append(assistant)
            elif col == "K" and death_list['knight2_death'] != 1:
                knight = Dialogue_img(x - 16,y - 16,128,68,NPS_KNIGHT,NPS_KNIGHT_2,0, knight_icon,knight_icon2,0,fait_knight,28,3,'knight2',knight2_t,False,False,True,128,68)
                animatedEntities2.add(knight)
                entities2.add(knight)
                fait_character.append(knight)
            elif col == "k" and death_list['knight3_death'] != 1:
                knight = Dialogue_img(x - 16,y - 16,128,68,NPS_KNIGHT,NPS_KNIGHT_2,0, knight_icon,knight_icon2,0,fait_knight,28,3,'knight3',knight3_t,True,False,True,128,68)
                animatedEntities2.add(knight)
                entities2.add(knight)
                fait_character.append(knight)
            elif col == "H" and death_list['knight4_death'] != 1:
                knight = Dialogue_img(x - 16,y - 16,128,68,NPS_KNIGHT,NPS_KNIGHT_2,0, knight_icon,knight_icon2,0,fait_knight,28,3,'knight4',knight2_t,False,False,True,128,68)
                animatedEntities2.add(knight)
                entities2.add(knight)
                fait_character.append(knight)
            elif col == "h" and death_list['knight5_death'] != 1:
                knight = Dialogue_img(x - 16,y - 16,128,68,NPS_KNIGHT,NPS_KNIGHT_2,0, knight_icon,knight_icon2,0,fait_knight,28,3,'knight5',knight3_t,True,False,True,128,68)
                animatedEntities2.add(knight)
                entities2.add(knight)
                fait_character.append(knight)
            elif col == "M" and death_list['mag_death'] != 1:
                mag = Dialogue_img(x - 16,y - 16,195,130,NPS_MAG,NPS_MAG2,0, mag_icon, mag_icon2,0,fait_mag,28,3,'mag',mag_t,False,False,True,195,130)
                animatedEntities2.add(mag)
                entities2.add(mag)
                fait_character.append(mag)
            x += PLATFORM_WIDTH #блоки платформы ставятся на ширине блоков
        y += PLATFORM_HEIGHT    #то же самое и с высотой
        x = 0                   #на каждой новой строчке начинаем с нуля

    for character in not_fait_character:
                        ...
                        if pygame.sprite.collide_rect(g,character):
                            if character.death != 1 and character.win != 1:
                                ...
                                if hero.health <= 0:
                                    hero.end = 5
                                    hero.winner = True
                                if death_list['knight2_win'] == 1 and death_list['knight3_win'] == 1 and death_list['knight4_win'] == 1 and death_list['knight5_win'] == 1 and death_list['assistant_quest3'] != 1:
                                    pf_knight.rect.y -= 520
                                    if death_list['fire_mag_win'] != 1:
                                        fire_mag.quest = 1
                                        fire_mag.dialogue = 1
                                if death_list['assistant_win'] == 1 and death_list['assistant_quest3'] == 1:
                                ...
                                if death_list['mag_win'] == 1:
                                    pf_knight.rect.y -= 520
                                    if death_list['assistant_quest3'] != 1 and death_list['witch_win'] != 1:
                                        w.quest = 1
                                        w.dialogue = 1
